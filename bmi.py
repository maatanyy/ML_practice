...
bmiData = bmiData[(bmiData['waist']!='-')&(bmiData['step_count']!='-')]   # 값 없는 것들 제외 시킴, 전체 322명중 123명 제외하고 199명만 남는다!!
...
bmiData[['gender','grade','waist','step_count','height','weight']] = scaler_x.fit_transform(bmiData[['gender','grade','waist','step_count','height','weight']])
scaler_y = MinMaxScaler()
bmiData['bmi'] = scaler_y.fit_transform(bmiData['bmi'].values.reshape(-1,1))


train_size = int(len(bmiData)*0.7)
train_set = bmiData[0:train_size]             #139
test_set = bmiData[train_size-seq_length:]    #67
...
with torch.no_grad():
    ...
    testY_inverse = scaler_y.inverse_transform(bmiData['bmi'][train_size:].values.reshape(-1, 1))
    #print(testY_inverse)
...
#plt.plot(np.arange(len(pred_inverse)), testY_inverse-pred_inverse,'ro', label = 'pred')
plt.plot(np.arange(len(pred_inverse)), pred_inverse,'ro', label = 'pred')               #60
plt.plot(np.arange(len(testY_inverse)), testY_inverse,'bo', label = 'true')             #60
#plt.plot(np.arange(len(pred_inverse)), pred_inverse, label = 'pred')               #60
#plt.plot(np.arange(len(testY_inverse)), testY_inverse, label = 'true')             #60
plt.title("Prediction")
plt.show()


#stats.pearsonr()    #피어슨 상관계수 함수   #앞 : 상관계수 숫자 ( 0.7 이상 정도면 매우 큰 상관관계, 뒤 : p-value (0.05 또는 0.01 이하 정도면 유의미한 상관이 있다고 간주)
